...
class TestEnterpriseUtils(unittest.TestCase):
    ...
    def test_course_exporter_export(self, mock_get_enterprise_course_runs):
        """
        ``CourseExporter``'s ``export`` produces a JSON dump of the course data.
        """
        mock_get_enterprise_course_runs.return_value.values.return_value = []
        mock_user = mock.MagicMock()
        mock_enterprise_configuration = mock.MagicMock()
        exporter = CourseExporter(mock_user, mock_enterprise_configuration)
        assert next(exporter.export()) == b'[]'
